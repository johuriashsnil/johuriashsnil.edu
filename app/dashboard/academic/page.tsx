...
import { UploadAcademicDataSection } from "./UploadRoutineSection";
...
const Page: React.FC = () => {
    ...
    const handleDeleteRoutine = async (id: string) => {
        try {
            const res = await axios.delete(`${BaseURL}/api/academic/${id}`);
            setData(data.filter((item) => item._id !== id));
            if (res.status === 200) {
                Swal.fire({
                    title: "Routine Deleted",
                    text: "Routine has been deleted successfully",
                    icon: "success",
